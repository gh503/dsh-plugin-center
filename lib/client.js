...
window.__ModuleLoader__.load({
  ...
  .dpm-panel{position:fixed;right:20px;bottom:124px;z-index:10000;width:320px;max-height:74vh;overflow:auto;box-sizing:border-box;padding:14px;border-radius:16px;border:1px solid rgba(23,30,60,.12);background:rgba(255,255,255,.94);color:#1a1d24;display:none;flex-direction:column;gap:10px;box-shadow:0 12px 36px rgba(30,40,80,.2);backdrop-filter:blur(14px);font-size:13px;line-height:1.5;}
  ...
  function buildPanel() {
    ...
    panel.innerHTML =
      ...
      '<div class="dpm-hint">默认仅 dsh 插件（取消勾选搜 npm 全库）；按近一月下载量排序；点击包名看版本发布页。安装/卸载后需重启服务（sudo systemctl restart dsh-web）生效。</div>';
